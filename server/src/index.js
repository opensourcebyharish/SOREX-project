...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', service: 'sorex-mission-control', time: new Date().toISOString() });
});
...
server.listen(config.port, () => {
  logEvent('system_start', 'SOREX Mission Control server started.', 'info', 'system');
  simulator.start();
  console.log('┌─────────────────────────────────────────────┐');
  console.log('│  🌊 SOREX — SOUTHERN OCEAN ROBOTIC EXPLORER │');

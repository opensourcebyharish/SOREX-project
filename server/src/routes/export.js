...
function buildFetch(req) {
  const deviceId = req.query.device || 'SOREX-001';
...
router.get('/csv', (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="sorex-export-${Date.now()}.csv"`);
  res.send(toCsv(rows));
});
...
router.get('/json', (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="sorex-export-${Date.now()}.json"`);

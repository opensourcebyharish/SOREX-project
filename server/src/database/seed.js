...
async function seed() {
  ...
  // ── Users ──────────────────────────────────────────────────────────────
  const adminHash = await bcrypt.hash('sorex2026', 10);
  const users = [
    { username: 'admin', email: 'admin@sorex.io', password_hash: adminHash, role: 'admin', created_at: iso(30 * DAY_MS) },
    { username: 'researcher1', email: 'maria@sorex.io', password_hash: adminHash, role: 'researcher', created_at: iso(28 * DAY_MS) },
    { username: 'op1', email: 'james@sorex.io', password_hash: adminHash, role: 'operator', created_at: iso(25 * DAY_MS) },
    { username: 'viewer1', email: 'guest@sorex.io', password_hash: adminHash, role: 'viewer', created_at: iso(20 * DAY_MS) },
  ];
  const uIds = [];
  for (const u of users) uIds.push(insert('users', u));
  console.log('Users seeded (admin / sorex2026).');

  // ── Device ─────────────────────────────────────────────────────────────
  insert('devices', {
    device_name: 'SOREX Prototype 01',
    device_id: 'SOREX-001',
  ...
  insert('devices', {
    device_name: 'SOREX Polar Prototype',
    device_id: 'SOREX-002',
  ...
  // ── System events ──────────────────────────────────────────────────────
  const events = [
    ['system_start', 'SOREX Mission Control server started.', 'info', 'system'],
    ['acquisition', 'Sensor acquisition started on SOREX-001.', 'info', 'stm32'],
  ...
  // ── Device health ──────────────────────────────────────────────────────
  const health = [
    ['SOREX-001', 'STM32', 'online', 'ok'],
    ['SOREX-001', 'ESP8266', 'online', 'RSSI -62 dBm'],
    ['SOREX-001', 'DS18B20', 'normal', 'temp_sensor'],
    ['SOREX-001', 'BMP180', 'normal', 'pressure_sensor'],
    ['SOREX-001', 'MPU6050', 'normal', 'imu'],
    ['SOREX-001', 'DS1307', 'normal', 'rtc'],
    ['SOREX-001', 'MicroSD', 'normal', '72% free'],
    ['SOREX-001', 'Battery', 'normal', '87%'],
    ['SOREX-001', 'Memory', 'normal', '61% used'],
  ];
  ...
  console.log('\n✔ Seed complete. Database ready at server/data/sorex.db');
